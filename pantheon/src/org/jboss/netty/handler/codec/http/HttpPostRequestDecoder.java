...
public class HttpPostRequestDecoder {
    ...
    */
    ...
    private void checkMultipart(String contentType)
        ...
        splitHeaderContentType(contentType, contentTypes);
        
        if (contentTypes.get(0).toLowerCase().startsWith(
                NHttpHeaders.Values.MULTIPART_FORM_DATA)) {
        	int boundaryIndex = getBoundaryIndex(contentTypes);
        	if (boundaryIndex >= 0){
        		String[] boundary = contentTypes.get(boundaryIndex) .split("=");
    ...
    private int getBoundaryIndex(List<String> list){
    	for (int i=0;i<list.size();i++){
    		if (list.get(i).toLowerCase().startsWith(NHttpHeaders.Values.BOUNDARY)){
    			return i;
    		}
    	}
    	return -1;
    ...
    */
    ...
    private void splitHeaderContentType(String sb, List<String> list){
    	 int size = sb.length();
         int aStart;
         int aEnd;
         int bStart;
    	...
    	list.add(sb.substring(aStart, aEnd));
    	splitHeaderContentType(sb.substring(bStart, bEnd) , list);
    }
